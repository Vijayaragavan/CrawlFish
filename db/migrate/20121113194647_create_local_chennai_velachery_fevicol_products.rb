class CreateLocalChennaiVelacheryFevicolProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,created_at) values('Fevicol','na','Good mobile shop','local','na','mobiles@gmail.com','134312','457633344', 'mambalam, chennai-24','128.083','56.098','chennai','velachery','9AM to 8.30PM','na',
'books_lists,mobiles_lists','variable','2012-05-14',now());
  ...
  CREATE TABLE IF NOT EXISTS local_chennai_velachery_fevicol_products (
  ...
  SQL
    ...
    execute <<-SQL
    CREATE TRIGGER t_Insert_local_chennai_velachery_fevicol_products AFTER INSERT ON local_chennai_velachery_fevicol_products
    FOR EACH ROW
    BEGIN
    ...
    SELECT vendor_id INTO v_VendorID FROM vendors_lists where vendor_name='fevicol' AND branch_name='velachery';
    ...
    execute <<-SQL
    CREATE TRIGGER t_Update_local_chennai_velachery_fevicol_products AFTER UPDATE ON local_chennai_velachery_fevicol_products
    FOR EACH ROW
    BEGIN
    ...
    SQL
    execute <<-SQL
    CREATE TRIGGER t_Delete_local_chennai_velachery_fevicol_products BEFORE DELETE ON local_chennai_velachery_fevicol_products
    FOR EACH ROW
    BEGIN
  ...
  def down
        ...
        execute "call p_drop_local_vendor('fevicol','chennai','velachery')"
        execute "DROP TRIGGER t_Delete_local_chennai_velachery_fevicol_products"
        execute "DROP TRIGGER t_Update_local_chennai_velachery_fevicol_products"
        execute "DROP TRIGGER t_Insert_local_chennai_velachery_fevicol_products"
        execute "DROP TABLE local_chennai_velachery_fevicol_products"
        system "rails destroy model local_chennai_velachery_fevicol_products"
  end
end
