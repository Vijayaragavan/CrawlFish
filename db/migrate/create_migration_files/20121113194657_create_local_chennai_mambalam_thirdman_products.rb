class CreateLocalChennaiMambalamThirdmanProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,created_at) values('ThirdMan','na','Good mobile shop','local','na','sayar-mobiles@gmail.com','34563456','457633344', 'mambalam, chennai-24','12.9202','79.1333','chennai','mambalam','9AM to 8.30PM','na',
'books_lists,mobiles_lists','variable','2012-05-14',now());
  ...
  CREATE TABLE IF NOT EXISTS local_chennai_mambalam_thirdman_products (
  ...
  SQL
    ...
    execute <<-SQL
    CREATE TRIGGER t_Insert_local_chennai_mambalam_thirdman_products AFTER INSERT ON local_chennai_mambalam_thirdman_products
    FOR EACH ROW
    BEGIN
    ...
    SELECT vendor_id INTO v_VendorID FROM vendors_lists where vendor_name='thirdman' AND branch_name='mambalam';
    ...
    execute <<-SQL
    CREATE TRIGGER t_Update_local_chennai_mambalam_thirdman_products AFTER UPDATE ON local_chennai_mambalam_thirdman_products
    FOR EACH ROW
    BEGIN
    ...
    SQL
    execute <<-SQL
    CREATE TRIGGER t_Delete_local_chennai_mambalam_thirdman_products BEFORE DELETE ON local_chennai_mambalam_thirdman_products
    FOR EACH ROW
    BEGIN
  ...
  def down
        ...
        execute "call p_drop_local_vendor('thirdman','chennai','mambalam')"
        execute "DROP TRIGGER t_Delete_local_chennai_mambalam_thirdman_products"
        execute "DROP TRIGGER t_Update_local_chennai_mambalam_thirdman_products"
        execute "DROP TRIGGER t_Insert_local_chennai_mambalam_thirdman_products"
        execute "DROP TABLE local_chennai_mambalam_thirdman_products"
        system "rails destroy model local_chennai_mambalam_thirdman_products"
  end
end
