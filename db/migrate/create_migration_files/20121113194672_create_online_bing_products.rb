class CreateOnlineBingProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,created_at) values('Bing','http://blog.flipkart.com/wp-content/uploads/2011/02/blog-logo-unit.png','Good online shop','online','www.flipkart.com','sayar-mobiles@gmail.com','1426509','454738382', 'tnagar, chennai-24','na','na','na','na','na','Online wallet, COD, 30 days replacement guarentee, EMI',
'books_lists,mobiles_lists','purchase','2012-05-14',now());
  ...
  CREATE TABLE IF NOT EXISTS online_bing_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_bing_products AFTER INSERT ON online_bing_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='bing';
    ...
    CREATE TRIGGER t_Update_online_bing_products AFTER UPDATE ON online_bing_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_bing_products BEFORE DELETE ON online_bing_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_bing_products"
	execute "call p_drop_online_vendor('bing')"
        execute "DROP TRIGGER t_Delete_online_bing_products"
        execute "DROP TRIGGER t_Update_online_bing_products"
        execute "DROP TRIGGER t_Insert_online_bing_products"
        execute "DROP TABLE online_bing_products"
        system "rails destroy model online_bing_products"
  end
end
