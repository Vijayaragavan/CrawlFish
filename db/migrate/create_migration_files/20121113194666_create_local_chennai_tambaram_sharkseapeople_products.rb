class CreateLocalChennaiTambaramSharkseapeopleProducts < ActiveRecord::Migration
...
'books_lists,mobiles_lists','variable','2012-05-14',now());
  ...
  CREATE TABLE IF NOT EXISTS local_chennai_tambaram_sharkseapeople_products (
  ...
  SQL
    ...
    execute <<-SQL
    CREATE TRIGGER t_Insert_local_chennai_tambaram_sharkseapeople_products AFTER INSERT ON local_chennai_tambaram_sharkseapeople_products
    FOR EACH ROW
    BEGIN
    ...
    SELECT vendor_id INTO v_VendorID FROM vendors_lists where vendor_name='sharkseapeople' AND branch_name='tambaram';
    ...
    execute <<-SQL
    CREATE TRIGGER t_Update_local_chennai_tambaram_sharkseapeople_products AFTER UPDATE ON local_chennai_tambaram_sharkseapeople_products
    FOR EACH ROW
    BEGIN
    ...
    SQL
    execute <<-SQL
    CREATE TRIGGER t_Delete_local_chennai_tambaram_sharkseapeople_products BEFORE DELETE ON local_chennai_tambaram_sharkseapeople_products
    FOR EACH ROW
    BEGIN
  ...
  def down
        ...
        execute "call p_drop_local_vendor('sharkseapeople','chennai','tambaram')"
        execute "DROP TRIGGER t_Delete_local_chennai_tambaram_sharkseapeople_products"
        execute "DROP TRIGGER t_Update_local_chennai_tambaram_sharkseapeople_products"
        execute "DROP TRIGGER t_Insert_local_chennai_tambaram_sharkseapeople_products"
        execute "DROP TABLE local_chennai_tambaram_sharkseapeople_products"
        system "rails destroy model local_chennai_tambaram_sharkseapeople_products"
  end
end
