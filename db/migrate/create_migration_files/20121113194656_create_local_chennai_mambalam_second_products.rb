class CreateLocalChennaiMambalamSecondProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,created_at) values('second','na','Good mobile shop','local','na','sayar-mobiles@gmail.com','23452345','457633344', 'mambalam, chennai-24','13.0780','80.2590','chennai','mambalam','9AM to 8.30PM','na',
'books_lists,mobiles_lists','variable','2012-05-14',now());
  ...
  CREATE TABLE IF NOT EXISTS local_chennai_mambalam_second_products (
  ...
  SQL
    ...
    execute <<-SQL
    CREATE TRIGGER t_Insert_local_chennai_mambalam_second_products AFTER INSERT ON local_chennai_mambalam_second_products
    FOR EACH ROW
    BEGIN
    ...
    SELECT vendor_id INTO v_VendorID FROM vendors_lists where vendor_name='second' AND branch_name='mambalam';
    ...
    execute <<-SQL
    CREATE TRIGGER t_Update_local_chennai_mambalam_second_products AFTER UPDATE ON local_chennai_mambalam_second_products
    FOR EACH ROW
    BEGIN
    ...
    SQL
    execute <<-SQL
    CREATE TRIGGER t_Delete_local_chennai_mambalam_second_products BEFORE DELETE ON local_chennai_mambalam_second_products
    FOR EACH ROW
    BEGIN
  ...
  def down
        ...
        execute "DELETE FROM local_chennai_mambalam_second_products"
        execute "call p_drop_local_vendor('second','chennai','mambalam')"
        execute "DROP TRIGGER t_Delete_local_chennai_mambalam_second_products"
        execute "DROP TRIGGER t_Update_local_chennai_mambalam_second_products"
        execute "DROP TRIGGER t_Insert_local_chennai_mambalam_second_products"
        execute "DROP TABLE local_chennai_mambalam_second_products"
        system "rails destroy model local_chennai_mambalam_second_products"
  end
end
